...
Сервис в методе GET api/tasks (чтение списка задач), использующий структуры данных в оперативной памяти, **плохо справляется с нагрузкой в 100 одновременных пользователей** с числом запросов около 4.3 в секунду.

Сервис в методе POST api/tasks (создание задачи), использующий структуры данных в оперативной памяти, **отлично справляется с нагрузкой в 1000 одновременных пользователей** с числом запросов около 3117 в секунду.
...
4.  **Пагинация:** Для снижения нагрузк ина сервер необходимо внедрить пагинацию данных при чтении списка задач.
